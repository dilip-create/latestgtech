...
class WithdrawTransactionList extends Component
{
    ...
    public function mount()
    {
        if(Session::get('auth')->role_name == 'Merchant'){
            $this->transactionlist = WithdrawTransaction::where('merchant_id', Session::get('auth')->merchant_id)->select('id', 'merchant_code', 'reference_id', 'systemgenerated_TransId', 'total', 'net_amount', 'mdr_fee_amount', 'customer_name', 'Currency', 'status', 'callback_url', 'customer_bank_name', 'bank_code', 'customer_account_number', 'created_at')->orderBy('id', 'DESC')->get();
            ...
            $this->transactionlist = WithdrawTransaction::where('agent_id', Session::get('auth')->agent_id)->select('id', 'merchant_code', 'reference_id', 'systemgenerated_TransId', 'total', 'net_amount', 'mdr_fee_amount', 'customer_name', 'Currency', 'status', 'callback_url', 'customer_bank_name', 'bank_code', 'customer_account_number', 'created_at')->orderBy('id', 'DESC')->get();
            ...
            $this->transactionlist = WithdrawTransaction::select('id', 'merchant_code', 'reference_id', 'systemgenerated_TransId', 'total', 'net_amount', 'mdr_fee_amount', 'customer_name', 'Currency', 'status', 'callback_url', 'customer_bank_name', 'bank_code', 'customer_account_number', 'created_at')->orderBy('id', 'DESC')->get();
